...
export default defineEventHandler(async (event): Promise<HomepageDTO> => {
  ...
  // Fetch the homepage data from the platform API
  const homepage = await $fetch<HomepageDTO>('page/homepage', {
    baseURL: APIPlatform,
    method: 'GET',
    headers: {
