<template>
  <div class="contact">
    <div class="contact_container">
      ...
      <div class="contact_container_object contact_container_object_split">
        <input
          class="contact_container_object_split_input"
          type="text"
          v-model="userName"
      ...
      <div class="contact_container_object">
        <input
          class="contact_container_object_input"
          type="mail"
          v-model="userEmail"
      ...
      <div class="contact_container_button">
        <button @click="sendMessage()">Envoyer</button>
        <div class="contact_container_button_socials">
          <a href="https://www.instagram.com/bellagp971/" target="blank">
            <svg
          ...
          <a
            href="https://www.facebook.com/people/Bella-GP/100077913630138/"
            target="_blank"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
...
export default {
  ...
  methods: {
    ...
    validateUserTel(input, minLength, maxLength) {
      if (!inputValidations.isNotEmpty(input)) {
      ...
      if (!inputValidations.validateLength(input, minLength, maxLength)) {
        throw new Error(
          `Le numéro de téléphone doit être compris entre ${minLength} et ${maxLength} nombres.`
        )
      }

      if (!inputValidations.validateNumericSpace(input)) {
        throw new Error(
          "Le numéro de téléphone n'est pas valide, il ne doit contenir que des nombres."
        )
      }
    ...
    validateMessage(input, minLength, maxLength) {
      ...
      if (!inputValidations.validateLength(input, minLength, maxLength)) {
        throw new Error(
          `Le message doit être compris entre ${minLength} et ${maxLength} caractères.`
        )
      }

      if (!inputValidations.validateBasicMessage(input)) {
        throw new Error(
          "Le message n'est pas valide, il ne doit pas contenir de caractère interdit."
        )
      }
    ...
    async sendMessage() {
      try {
        if (this.sendMessageTimeout === true) {
          throw new Error('Veuillez attendre quelques secondes')
        }

        this.sendMessageTimeout = true
        this.validateName(this.userName, 1, 100)
        this.validateSurname(this.userSurname, 1, 100)
        this.validateUserEmail(this.userEmail)
        this.validateUserTel(this.userTel, 6, 12)
        this.validateMessage(this.messageObject, 1, 500)
        this.validateMessage(this.messageContent, 1, 5000)

        const response = await fetch('/api/account/support-message', {
          method: 'POST',
          headers: {
